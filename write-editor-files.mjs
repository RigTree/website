...
function ArrayEditor({ items, onAdd, onRemove, onUpdate, renderItem, addLabel }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-sm)' }}>
      ...
      <button onClick={onAdd} className="btn-ghost" style={{ width: '100%', justifyContent: 'center', border: '1px dashed var(--border-hover)', padding: '0.625rem', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
        <Plus size={13} />
        {addLabel}
      </button>
    </div>
...
function Section({ title, open, onToggle, children }) {
  return (
    <div style={{ borderBottom: '1px solid var(--border)' }}>
      <button
        onClick={onToggle}
        style={{ display: 'flex', width: '100%', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 0', fontSize: '0.8rem', fontWeight: 600, fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.06em', color: open ? 'var(--text-primary)' : 'var(--text-secondary)', background: 'none', border: 'none', cursor: 'pointer', transition: 'color 0.15s' }}
      ...
      {open && (
        <div style={{ paddingBottom: 'var(--space-md)', display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
          {children}
        </div>
      )}
    </div>
...
export default function ComputersStep({ data, onChange }) {
  ...
  const addComputer = () => {
    ...
    onChange((prev) => ({ ...prev, computers: [...prev.computers, newComputer] }));
    setEditingIdx(data.computers.length);
  };

  const removeComputer = (idx) => {
    onChange((prev) => ({ ...prev, computers: prev.computers.filter((_, i) => i !== idx) }));
    ...
    <div>
      {data.computers.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '3rem 2rem', border: '1px dashed var(--border-hover)', borderRadius: 'var(--radius-lg)' }}>
          <Monitor size={32} style={{ color: 'var(--text-secondary)', margin: '0 auto 0.875rem', opacity: 0.6 }} />
          <p style={{ fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.4rem' }}>No computers yet</p>
          <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', marginBottom: '1.5rem', lineHeight: 1.6 }}>Add your desktop, laptop, or any other machine.</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            <div key={comp.id} className="card" style={{ overflow: 'hidden' }}>
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); removeComputer(idx); }}
                    className="btn-ghost"
          ...
          <button onClick={addComputer} className="btn-secondary" style={{ width: '100%', justifyContent: 'center', marginTop: '0.25rem' }}>
            <Plus size={14} /> Add Another Computer
          </button>
        </div>
      )}
    </div>
